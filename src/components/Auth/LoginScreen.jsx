...
import { useLoginMutation, useSocialsignupMutation } from '@/Services/HandleAPI';
...
export default function LoginScreen() {
    ...
    const handleGoogleLogin = async () => {
        try {
            setApiErrorState("");
            console.log("🔍 [Debug] Google Login Started");
            
            const result = await signInWithPopup(auth, googleProvider);
            const user = result.user;
            
            // Get the credential to access the Google Access Token
            const credential = GoogleAuthProvider.credentialFromResult(result);
            const googleToken = credential?.accessToken;

            console.log("✅ [Debug] Firebase Sign-In Success:", { uid: user.uid, email: user.email });

            if (!googleToken) {
                throw new Error("Could not retrieve Google access token from result.");
            }

            // Prepare payload for backend according to user specifications
            const payload = {
                socialSite: "Google", 
                accessToken: googleToken
            };
            
            console.log("📤 [Debug] Sending Corrected Payload to Backend:", { ...payload, accessToken: "REDACTED" });

            const res = await socialsignup(payload).unwrap();
            console.log("🎉 [Debug] Backend Social Signup Success:", res);

            const token = res?.token || res?.access_token || res?.data?.token || res?.data?.access_token;
            
            if (token) {
                localStorage.setItem("token", token);
                console.log("💾 [Debug] Backend JWT stored in localStorage");
                toast.success("Login Successful!");
                Navigate("/home");
            } else {
                console.warn("⚠️ [Debug] No token found in backend response");
                toast.error("Token not found. Please try again.");
            }
        } catch (error) {
            console.error("❌ [Debug] Google Login Error:", error);
            
            // Detailed error logging for backend 422/401 etc
            if (error?.data) {
                console.error("📦 [Debug] Backend Error Data:", error.data);
                const backendMsg = error?.data?.message || (typeof error.data === 'string' ? error.data : JSON.stringify(error.data));
                setApiErrorState(backendMsg);
            }
            ...
            <div className="relative z-10 w-full max-w-150 min-h-fit bg-white rounded-xl shadow-2xl p-6 sm:p-12 flex flex-col justify-center items-center ml-0 lg:ml-[2%] my-8 sm:my-0">
                ...
                <div className="flex gap-4 mb-8">
                    <button 
                        ...
                        className="w-12 h-12 rounded-full border border-gray-200 flex items-center justify-center hover:bg-gray-50 transition-colors bg-white disabled:opacity-50"
                    >
