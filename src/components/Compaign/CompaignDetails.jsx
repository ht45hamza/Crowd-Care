...
export default function CompaignDetails() {
    ...
    const handleImageUpload = (e) => {
        ...
        setRawImageFiles(prev => [...prev, ...files]);
    };

    const removeImage = (indexToRemove) => {
        setImages(images.filter((_, index) => index !== indexToRemove));
        setRawImageFiles(rawImageFiles.filter((_, index) => index !== indexToRemove));
    };

    const handleContinue = () => {
        if (!amount || !durationFrom || !durationTo || images.length === 0 || !description) {
            alert("Please fill out all amount details, attach at least one image, and enter a description.");
            return;
        }

        navigate('/compaign-review', {
            state: {
                ...prevData,
                amount,
                    ...
                    <div className="w-full max-w-[1050px] mx-auto mt-4 sm:mt-6 px-4 sm:px-6 flex flex-col md:flex-row gap-6 lg:gap-16 flex-1">
                        ...
                        <div className="flex-1 flex flex-col w-full min-h-fit md:min-h-170">
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 mb-6 sm:mb-8 pt-2">
                                ...
                                <div className="flex-1 flex gap-2 h-14 sm:h-[65px] border border-gray-100 shadow-[0_2px_15px_rgba(0,0,0,0.03)] rounded-2xl px-4 items-center bg-white transition-colors focus-within:ring-1 focus-within:ring-gray-300">
                            ...
                            <div className="mb-6 sm:mb-8">
                                <div className="flex justify-between items-center mb-3">
                                    <h3
                                        className="font-bold text-[15px] sm:text-[16px] tracking-wide"
                                        style={{ color: Colors.primary }}
                                    >
                                        Attach Images {images.length}/8
                                    </h3>
                                </div>
                                    ...
                                    <div className="flex-1 border border-gray-200 rounded-3xl sm:rounded-4xl p-4 sm:p-6 flex flex-wrap gap-3 sm:gap-4 shadow-sm bg-white min-h-[100px] sm:min-h-35">
                                        ...
                                        {images.map((imgSrc, index) => (
                                            <div key={index} className="relative w-16 h-10 sm:w-21.25 sm:h-13.75 rounded-lg sm:rounded-xl overflow-hidden shadow-sm">
                                                ...
                                                >
                                                    <X size={8} sm:size={10} style={{ color: Colors.primary }} />
                                                </button>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <div className="mb-0 flex-1 flex flex-col mt-4">
                                <h3
                                    className="font-bold text-[16px] tracking-wide mb-3"
                                    style={{ color: Colors.primary }}
                                >
                                    Description
                                </h3>
